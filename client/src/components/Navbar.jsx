...
const Navbar = () => {
  ...
  return (
    ...
    xl:px:32 py-4 text-gray-600 border-b border-borderColor transition-all sticky top-0 z-50
    ...
    `}>
      ...
      <div className={`max-sm:fixed max-sm:h-screen max-sm:w-full max-sm:top-16 
        max-sm:border-t border-borderColor right-0 flex flex-col sm:flex-row item-start sm:items-center gap-4 
        ...
        <div className='flex max-sm:flex-col items-start sm:items-center gap-6'>
          ...
          <button className='cursor-pointer px-8 py-2 bg-primary
           hover:bg-primary-dull transition-all text-white rounded-lg' onClick={()=> {user ? logout() : setShowLogin(true)}}>{user ? 'Logout' : 'Login'}</button>
        </div>
      </div>
